...
void set_latency(FemuCtrl *n)
{
    ...
    for(int i=0; i < n->num_zones; i++, zone++)
    {
        if (n->zone_array->d.zone_flash_type == TLC) {
            n->zone_array->d.rd_lat_ns = TLC_UPPER_PAGE_READ_LATENCY_NS;
            n->zone_array->d.wr_lat_ns = TLC_UPPER_PAGE_WRITE_LATENCY_NS;
            n->zone_array->d.er_lat_ns = TLC_BLOCK_ERASE_LATENCY_NS;
            n->zone_array->d.chnl_pg_xfer_lat_ns = TLC_CHNL_PAGE_TRANSFER_LATENCY_NS;
        } else if (n->zone_array->d.zone_flash_type == QLC) {
            n->zone_array->d.rd_lat_ns  = QLC_UPPER_PAGE_READ_LATENCY_NS;
            n->zone_array->d.wr_lat_ns  = QLC_LOWER_PAGE_WRITE_LATENCY_NS;
            n->zone_array->d.er_lat_ns  = QLC_BLOCK_ERASE_LATENCY_NS;
            n->zone_array->d.chnl_pg_xfer_lat_ns = QLC_CHNL_PAGE_TRANSFER_LATENCY_NS;
        } else if (n->zone_array->d.zone_flash_type == MLC) {
            n->zone_array->d.rd_lat_ns = MLC_UPPER_PAGE_READ_LATENCY_NS;
            n->zone_array->d.wr_lat_ns = MLC_LOWER_PAGE_WRITE_LATENCY_NS;
            n->zone_array->d.er_lat_ns = MLC_BLOCK_ERASE_LATENCY_NS;
